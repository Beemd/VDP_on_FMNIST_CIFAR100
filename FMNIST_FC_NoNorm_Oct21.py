...
from VDPLayers_FMNIST_FC import VDP_Flatten, VDP_Conv2D, VDP_Relu, VDP_Maxpool, VDP_FullyConnected, VDP_Softmax
import logging
import torchattacks
...
# Load Model
parser.add_argument('--load_model', type=str, default='../Results/checkpoints/FaMNIST_eVI_FC_model_041021_1000.pth', help='Path to a previously trained model checkpoint')
parser.add_argument('--data_path', type=str, default='../../../data/', help='Path to save dataset data')
# Loss Function Parameters
parser.add_argument('--tau1', type=float, default=0.001, help='KL Weight Term') 
parser.add_argument('--tau2', type=float, default=0.001, help='KL Weight Term') 
parser.add_argument('--tau3', type=float, default=0.001, help='KL Weight Term') 
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.fc1 = VDP_FullyConnected(28*28, 500, input_flag=True)
        self.fc2 = VDP_FullyConnected(500, 100)
        self.relu = VDP_Relu()
        self.fc3 = VDP_FullyConnected(100, 10)        
        self.softmax = VDP_Softmax(1)

        
    def forward(self, x):
        mu_flat = torch.flatten(x, start_dim=1)
        muf, sigmaf = self.fc1(mu_flat)
        muf, sigmaf = self.relu(muf, sigmaf)
        muf, sigmaf = self.fc2(muf, sigmaf)
        muf, sigmaf = self.relu(muf, sigmaf)
        muf, sigmaf = self.fc3(muf, sigmaf)
    ...
    def batch_loss(self, output_mean, output_sigma, target):
        ...
        loss_value = 0.5*neg_log_likelihood + (self.tau1 * self.fc1.kl_loss_term() +
                                           self.tau2 * self.fc2.kl_loss_term() +
                                           self.tau3 * self.fc3.kl_loss_term())
        return loss_value
...
def train(args, model, optimizer, train_loader, epoch):
    ...
    torch.save(model.state_dict(), '../Results/checkpoints/FaMNIST_eVI_FC_model_041021_1000.pth')
    torch.save(optimizer.state_dict(), '../Results/checkpoints/FaMNIST_eVI_FC_optimizer_041021_1000.pth')
...
def test(args, model, test_loader, noise_std):
    ...
    with open('../Results/withNoise/FaMNIST_FC_041021_1000/FaMNIST_FC_GN_Results_041021_1000_{}'.format(noise_std), 'wb') as pf:
        print('saving')
...
def test_adv(args, model, test_loader, attack, eps):
    ...
    with open('../Results/withAdv/FaMNIST_FC_041021_1000/FaMNIST_FC_AdvAttk_Results_041021_1000_{}_{}'.format(
...
def test_cw(args, model, test_loader, attack, eps):
    ...
    with open('../Results/withAdv/FaMNIST_FC_290621_1700/FaMNIST_FC_Targeted_CW_041021_1000_{}_{}'.format(
        eps, args.adv_trgt), 'wb') as pf:
        print('saving')
        pickle.dump([actual, adv_images, target, prediction, variance, corrmu, imu, corrvar, ivar, corr_adv, i_adv, corr_target, i_target], pf)
        pf.close()
...
def main():
    ...
    if args.testing==False:
        ...
        if args.add_noise==True:
            ...
            for eps in epsilon:
                if eps==0:
                    attack = torchattacks.PGD(new_model, eps=30/255, alpha=4/255, steps=20, random_start=False)
                elif eps==1:
